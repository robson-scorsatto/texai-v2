...
## Sprint 16 — Integrações reais prontas para ativação (WhatsApp Meta + Stripe)

Objetivo explícito desta sprint (confirmado com Robson): deixar toda a estrutura de integração real construída, testada e documentada, para que a ativação de WhatsApp real e cobrança real seja feita **inteiramente pelo `/admin`**, sem qualquer trabalho de código posterior — só colar credenciais quando Robson decidir. Referência: `TEXAI 2.0 — MASTER DEVELOPMENT PROMPT`; decisão de escopo confirmada nesta sprint: credenciais de WhatsApp (Meta Cloud API) são configuradas por clínica individualmente, credenciais do Stripe são únicas para toda a plataforma.

| # | Requisito | Implementação | Status | Teste |
|---|---|---|---|---|
| 1 | Schema de integrações (credenciais criptografadas) | `src/db/schema/integrations.ts` — `clinic_integrations` (por clínica, hoje só `meta_whatsapp`), `platform_integrations` (única por provedor, hoje só `stripe`), `stripe_webhook_events` (log append-only de eventos recebidos). `src/lib/crypto/secret-box.ts` — AES-256-GCM com IV único por valor e verificação de tampering via auth tag, chave vinda de nova env var `INTEGRATIONS_ENCRYPTION_KEY` (separada de `SESSION_SECRET`) | ✅ Feito | Migration `drizzle/0009_tense_umar.sql` aplicada com sucesso; `tests/integrations.test.ts` — bloco "Crypto — secret-box round trip" |
| 2 | Provider real — Meta WhatsApp Cloud API | `src/lib/messaging/providers/meta-cloud-provider.ts` — implementação real de `MessageProvider` chamando `graph.facebook.com`. `provider-factory.ts` — `getMessageProviderForClinic(clinicId)` escolhe automaticamente Meta real (se configurado E ativo) ou o mock (Sprint 11), nunca lança erro | ✅ Feito | `tests/integrations.test.ts` — bloco "WhatsApp integration — provider factory fallback" |
| 3 | Provider real — Stripe (pagamento) | `src/lib/billing/providers/payment-provider.ts` (interface), `stripe-provider.ts` (Checkout Session real, usando `price_data` dinâmico — não exige criar Price no Dashboard do Stripe manualmente), `mock-payment-provider.ts` (fallback). Endpoint `/api/webhooks/stripe` recebe e grava eventos (verificando assinatura), mas **não age automaticamente** sobre `subscriptions` — decisão deliberada de escopo (ver `docs/GUIA_INTEGRACAO.md`) | ✅ Feito | `tests/integrations.test.ts` — bloco "Stripe integration — provider factory fallback" |
| 4 | Service layer + server actions | `src/lib/integrations/integrations-service.ts` — leituras nunca retornam o segredo em claro, só status mascarado (`••••••••ab12`). WhatsApp: OWNER da própria clínica OU platform-admin podem gerenciar (confirmado com Robson). Stripe: platform-admin apenas (conta única da plataforma). `src/app/actions/integrations-actions.ts` — mesmo padrão `ActionResult<T>` de todos os módulos anteriores | ✅ Feito | `tests/integrations.test.ts` — blocos "access control" (WhatsApp e Stripe) e "server action layer" |
| 5 | UI — configuração no /admin | Nova ação "WhatsApp" expansível por clínica na tabela de clínicas do `/admin` (`admin-clinic-whatsapp.tsx`); novo card "Integrações — Pagamento (Stripe)" no `/admin` (`admin-stripe-config.tsx`). Ambos: formulário de credenciais + toggle ativar/desativar + status mascarado, nunca exibindo o segredo salvo de volta | ✅ Feito | Verificado via `next build`; sem E2E de navegador nesta sessão |
| 6 | Testes | `tests/integrations.test.ts` (23 casos): round-trip de criptografia, detecção de adulteração (tampering), mascaramento de segredo, controle de acesso (dono da própria clínica vs. outra clínica vs. platform-admin), fallback automático mock↔real nos dois providers, isolamento entre clínicas nas credenciais de WhatsApp, validação de formato da chave do Stripe, não vazamento de segredo pela camada de server actions | ✅ Feito | 153/153 testes passando no total (`npm run test`, executado em lotes nesta sessão por limite de tempo da ferramenta de shell — todos os 15 arquivos de teste passaram) |
| 7 | Guia de ativação | Novo `docs/GUIA_INTEGRACAO.md` — passo a passo em português para obter as credenciais reais (Meta for Developers, Stripe Dashboard) e ativá-las via `/admin`, sem necessidade de tocar em código | ✅ Feito | Revisão manual do documento |

### Pendências conhecidas do Sprint 16 — leitura obrigatória antes de qualquer uso em produção

- **`INTEGRATIONS_ENCRYPTION_KEY` usa um valor padrão inseguro em desenvolvimento.** Antes de salvar qualquer credencial real em produção, é **obrigatório** gerar um valor real (`openssl rand -base64 32`) e configurá-lo como variável de ambiente — do contrário, qualquer pessoa com acesso a este repositório consegue descriptografar as credenciais salvas.
- **WhatsApp real ainda não suporta templates pré-aprovados pela Meta** — só mensagens de texto livre, que só chegam dentro da janela de 24h de atendimento ao cliente. Suficiente para responder a um paciente que acabou de mandar mensagem; não é suficiente para iniciar uma conversa "a frio" (ex.: lembrete de consulta sem interação recente).
- **O checkout do Stripe ainda não está conectado à interface do cliente** — o provider sabe criar a sessão de checkout, mas nenhum botão em `/configuracoes/plano` chama essa função ainda. Fica para quando Robson decidir ativar cobrança de verdade.
- **O webhook do Stripe só registra eventos — não sincroniza `subscriptions` automaticamente.** Decisão deliberada: as regras de negócio para "o que fazer quando um pagamento falha" ainda não foram definidas.
- Sem suporte a PIX/boleto (Stripe suporta no Brasil, mas não foi configurado nesta sprint).
- Sem teste E2E de navegador para os formulários de configuração no `/admin` (mesma lacuna estrutural documentada nos sprints anteriores).
- Mesma pendência operacional dos Sprints 7-15 sobre `createClinic()` não deduplicar por nome ao reexecutar `npm run db:seed` em um banco já semeado.
